...
- **Compartment recovery (the headline).** Using the published marker→domain panel (Grasso et al. 2025, *Eur. J. Immunol.*), High-High LISA hotspots for the four compact compartments form a clean block structure: **mean Jaccard 0.49 within vs 0.02 between** (follicle, germinal center, T-zone, medulla), each block completely segregated (0.00), with germinal centers nested in follicles — matching known architecture. Thin structures (blood/lymphatic vessels, the B–T interface) do not form hotspots — an honest limit of a compartment-hotspot method. A non-circular external check against the pathologist H&E germinal-centre annotation (Chrysalis/cell2location, same section) is the identified next step. See `docs/methodology.md` §8.
